...
def calculate_simple_classification_predictions(model_constructor, data_folder, out_folder, max_clicks, fast):
    """
    ...
    """
    if fast:
        fast_calculate_simple_classification_predictions(model_constructor, data_folder, out_folder, max_clicks)
        return
...
def fast_calculate_simple_classification_predictions(model_constructor, data_folder, out_folder, max_clicks):
    """
    model_constructor: regression model. Has fit(x, y) and predict(x) methods.
    data_folder: str. Directory, containing files "day_i.json" where i in range(DAYS_NUMBER).
    out_folder: str. Directory, to save results. DAYS_NUMBER - 1 files will be created.

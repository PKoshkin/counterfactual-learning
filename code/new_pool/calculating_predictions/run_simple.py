...
def calculate_regression(args):
    ...
    if args.trashold is not None:
        raise ArgumentException("\"trashold\" argument is not valid for binary classification")
...
def calculate_binary_classification(args):
    if args.trashold is None:
        raise ArgumentException("\"trashold\" argument is required for binary classification")
    if args.max_clicks is not None:
        raise ArgumentException("\"max_clicks\" argument is valid only for classification")
    if args.model.lower() == "catboost":
...
def run():
    ...
    parser.add_argument("--trashold", type=float)
